...
async fn create_narrative(app: &axum::Router, cookie: &str, case_id: Uuid) -> Result<StatusCode> {
	let body = json!({
		"data": {
			"case_id": case_id,
...
#[serial]
#[tokio::test]
async fn test_admin_can_create_narrative() -> Result<()> {
	...
	let status = create_narrative(&app, &cookie_header(&token.to_string()), case_id).await?;
...
#[serial]
#[tokio::test]
async fn test_viewer_cannot_create_narrative() -> Result<()> {
	let mm = init_test_mm().await?;
	let seed = seed_org_with_users(&mm, "adminpwd", "viewpwd").await?;
	...
	let status = create_narrative(&app, &cookie_header(&viewer_token.to_string()), case_id).await?;
